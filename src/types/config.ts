...
export interface SecurityConfig {
  ...
  /**
   * Origin of your web app (e.g. `'https://example.com'`), the only one the
   * default OAuth callback template posts the session to. Required to
   * register an OAuth provider without a custom `template`.
   */
  oauthTargetOrigin?: string;
  /**
...
export interface EmailTemplateData extends Record<string, any> {
  /**
   * Public URL of your app without trailing slash, e.g.
   * `'https://example.com'`. The default templates build their links with it.
   * Required when they are used to send emails: links built from the request's
   * `Host` header would let anyone send reset links pointing to their host.
   */
  baseUrl?: string;
}
...
export interface ProviderConfig {
   ...
   * You can copy the default from `templates/oauth/authCallback.njk`. Never
   * post it with the targetOrigin `'*'`: any site that opens the popup would
   * receive the session.
   *
   ...
   *  - payloadJson: `{ error, session, link }` as JSON that is safe to put
   *    inside a `<script>` with `| safe`
   *  - targetOriginJson: `security.oauthTargetOrigin` as such JSON
   */
  template?: string;
